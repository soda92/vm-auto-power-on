...
def load_config():
    ...
    if os.path.exists(SECRETS_FILE):
        ...
        print(f"[!] Warning: Using {DEMO_SECRETS_FILE}. Please create {SECRETS_FILE} for real credentials.")
    else:
...
def scp_file(host, user, local_path, remote_path, sudo=False):
    ...
    if sudo:
        ...
        subprocess.check_call(f"scp -o StrictHostKeyChecking=no {local_path} {user}@{host}:{tmp_path}", shell=True)
        run_remote(host, user, f"mv {tmp_path} {remote_path}", sudo=True)
    else:
        subprocess.check_call(f"scp -o StrictHostKeyChecking=no {local_path} {user}@{host}:{remote_path}", shell=True)
